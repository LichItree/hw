...
{   
    ...
    return ((AB[0]*AP[1]-AB[1]*AP[0]>0) &&(BC[0]*BP[1]-BC[1]*BP[0]>0)&&(CA[0]*CP[1]-CA[1]*CP[0]>0))|| ((AB[0]*AP[1]-AB[1]*AP[0]<0) && (BC[0]*BP[1]-BC[1]*BP[0]<0)&& (CA[0]*CP[1]-CA[1]*CP[0]<0));
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    int max_Y = std::ceil(std::max(v[0][0],std::max(v[1][0],v[2][0])));
   ...
   for(int x = min_X; x<max_X+1;x++){
       for(int y = min_Y; y<max_Y+1;y++){
           if(MSAA){
               ...
               pos.push_back({1.0,1.0});
               int count =0;
               float mindep = INFINITY;
                for(int c=0;c<4;c++){
                    if( insideTriangle(x+pos[c][0],y+pos[c][1],t.v)){
                        count++;
                ...
                if(depth_buf[get_index(x,y)]>mindep){
                        Vector3f color = t.getColor()*count/4.0;
                        Vector3f point;
                        point << (float)x,(float)y,mindep;
